...
function Home() {
	...
	const crossAreaPointsRef = useRef([]); // 记录跨区域的点
	const crossAreaLinesRef = useRef([]); // 记录跨区域的线
	const currentLoadedGroupId = useRef(0); // 当前加载到的组号
	...
	const loadMore = useCallback(async () => {
		...
		currentLoadedGroupId.current = currentLoadedGroupId.current + 1;
		const {
			data: { scalars, points, lines },
		} = await get("/getData?type=0&groupId=" + currentLoadedGroupId.current); // 通过http接口获取scalars,points,lines数据
		const crossAreaLinePiece = crossAreaLinesRef.current.reduce(
			(acc, v, idx, arr) => {
				if (idx % 3 === 0) {
					if (
						(arr[idx + 1] === currentLoadedGroupId.current &&
							arr[idx + 2] < currentLoadedGroupId.current) ||
						(arr[idx + 2] === currentLoadedGroupId.current &&
							arr[idx + 1] < currentLoadedGroupId.current)
					) {
						acc.concat(2, arr[idx + 1], arr[idx + 2]);
					}
				}
				return acc;
			},
		...
		linesCopyRef.current = linesCopyRef.current
			.concat(...lines)
			.concat(...crossAreaLinePiece);
		linesRef.current = deepClone(linesCopyRef.current);
		// 此处需要考虑其他子树处于折叠状态后的显示效果
		// linesRef = ?
		// displayPoints = ？
		heightListRef.current = heightListRef.current.concat(...scalars);
		displayPointsRef.current = heightListRef.current.map((v, idx) => idx);
		sonNodeMap.current = {}; // 更新后清空
		polydata.getLines().setData(linesCopyRef.current); // 设定lines数据
		polydata.getPoints().setData(polydata.getPoints().getData().concat(points)); // 设定points数据
		closeList.current = [];
		...
		for (let i = 0; i < polydata.getNumberOfPoints(); i++) {
			...
			if (!!pointsActors[i]) pointsActors[i].delete();
	...
	const collapseLines = useCallback(
		async (id) => {
			 ...
			 * 分布加载重点内容！！！！！！！！！！！！！！！
			 */
			if (crossAreaPointsRef.current.indexOf(id) >= 0) {
				// 存在跨区域情况，需要加载
				loadMore();
				return;
			}
	...
	useEffect(() => {
		if (!context.current) {
			...
			(async function () {
				...
				let res = await import("../../assets/st/crossAreaPoints");
				crossAreaPointsRef.current = res.default;
				res = await import("../../assets/st/crossAreaLines");
				crossAreaLinesRef.current = res.default;
	...
	return (
		<div style={{ border: "red" }}>
			...
			<div className="white-board">
				...
				<Button onClick={loadMore}>加载下一组</Button>
